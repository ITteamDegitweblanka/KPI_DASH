...
exports.getTeamPerformance = exports.finalizePerformanceReview = exports.deletePerformanceReview = exports.updatePerformanceReview = exports.createPerformanceReview = exports.getEmployeePerformance = void 0;
...
const getEmployeePerformance = async (req, res) => {
    ...
    if (req.user?.id !== employeeId && req.user?.role !== types_1.UserRole.ADMIN && req.user?.role !== types_1.UserRole.SUPER_ADMIN) {
        ...
        if (!employee?.teamId) {
            throw new api_error_1.ApiError(http_status_codes_1.StatusCodes.FORBIDDEN, 'Employee is not part of any team');
        }
    ...
    try {
        ...
        if (status) {
            where.status = status; // Cast to any since we've already validated the status
        }
        ...
        throw new api_error_1.ApiError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to retrieve employee performance reviews');
    }
};
...
const createPerformanceReview = async (req, res) => {
    const { revieweeId, reviewerId, strengths = {}, areasForImprovement = {} } = req.body;
    try {
        ...
        if (!reviewee) {
            throw new api_error_1.ApiError(http_status_codes_1.StatusCodes.NOT_FOUND, 'Reviewee not found');
        }
        ...
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError) {
            if (error.code === 'P2002') {
                throw new api_error_1.ApiError(http_status_codes_1.StatusCodes.CONFLICT, 'A performance review already exists for this employee and review period');
            }
        }
        throw error;
    }
...
const updatePerformanceReview = async (req, res) => {
    ...
    try {
        ...
        const isAdmin = req.user?.role === types_1.UserRole.ADMIN || req.user?.role === types_1.UserRole.SUPER_ADMIN;
...
const deletePerformanceReview = async (req, res) => {
    ...
    try {
        // Check if review exists
        const review = await prisma_1.prisma.performanceReview.findUnique({
            where: { id },
            select: {
                reviewerId: true,
                revieweeId: true,
        ...
        if (!isAdmin && !isReviewer && !isReviewee) {
            throw new api_error_1.ApiError(http_status_codes_1.StatusCodes.FORBIDDEN, 'You are not authorized to delete this review');
        }
        // Prevent deletion of finalized reviews for non-admins
        if (review.isFinalized && !isAdmin) {
            throw new api_error_1.ApiError(http_status_codes_1.StatusCodes.FORBIDDEN, 'Only administrators can delete finalized reviews');
        }
...
const finalizePerformanceReview = async (req, res) => {
    ...
    try {
        ...
        if (review.isFinalized) {
            throw new api_error_1.ApiError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'This review has already been finalized');
        }
...
const getTeamPerformance = async (req, res) => {
    ...
    try {
        ...
        const reviews = await prisma_1.prisma.performanceReview.findMany({
            where: {
                revieweeId: { in: teamMembers.map((m) => m.id) },
        ...
        const memberPerformance = teamMembers.map((member) => {
            ...
            if (memberReviews.length > 0) {
                ...
                const totalScore = memberReviews.reduce((sum, review) => {
                    ...
                    if (review.metrics && review.metrics.length > 0) {
                        const metricsScore = review.metrics.reduce((metricSum, metric) => {
            ...
            const latestReview = [...memberReviews].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
            return {
                id: member.id,
        ...
        const membersWithReviews = memberPerformance.filter(m => m.reviewCount > 0);
        const totalScore = membersWithReviews.reduce((sum, member) => sum + (member.score || 0), 0);
        ...
        const categoryAverages = reviews.reduce((acc, review) => {
        ...
        const responseData = {
            ...
            data: {
                ...
                scoreDistribution: {
                    excellent: memberPerformance.filter((m) => m.score && m.score >= 90).length,
                    good: memberPerformance.filter((m) => m.score && m.score >= 75 && m.score < 90).length,
                    average: memberPerformance.filter((m) => m.score && m.score >= 60 && m.score < 75).length,
                    needsImprovement: memberPerformance.filter((m) => m.score && m.score < 60).length
                },
        ...
        throw new api_error_1.ApiError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to retrieve team performance data');
    }
};
